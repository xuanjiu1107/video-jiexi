...
//用户设置的防盗域名
define('USER_DOMAIN', $user['domain']);
//用户设置是否跳转至官网
...
define('VERSION', 'V2.3.5.520190525');
define('API_YOU_PATH','AK%3D3mN01KfDZ2h-Eop1yUpluSw12Y-kJ0R2KOfWK4ICfQ9za');

//播放器路径
define('PLAYPATH', $user['path']);
//当前环境
...
define('C_ROOT_KEY', md5(USER_TOKEN . 'nxflv.com'));
...
class __abose{
		private function get_api(){
				return self::unlockString(API_YOU_PATH);
		}
		public function is_referer(){		
				if (REFERER_URL == '') return true;else{
		...
		public function geturl($url)
		{
				...
				curl_setopt($curl, CURLOPT_USERAGENT, 'NxFlv');
		...
		public function other_loca($json,$u)
		{
				$jsons = json_decode($json, true);
				if ($jsons['code'] == '200' && $jsons['type'] == 'xml'){
						$xml_url = "";
						$xml="";
						...
						$fileName = $this->__cache($u);
						file_put_contents($fileName,$xml_url);
						$json_loca = json_encode(array(
								'code' => '200',
								'list' => $jsons['type'],
								'type' => 'xml',
								'url' => http_url().PLAYPATH.'/api.php?xml='.$fileName,
								'msg' => 'pc',
								'live' => '0',
								'player' => 'ckplayer'));
				}else{
		...
		public static function unlockString($txt,$key='NuoXun'){
			$txt = urldecode($txt);
		...
		private function pass(){
				$pass = '1d566de5253f328035dff5cb8b57f593';
				return $pass;
		}
